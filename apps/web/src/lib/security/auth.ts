...
/** Validate x-api-key against env bootstrap key or hashed keys table. */
export function verifyApiKey(headerValue: string | null): boolean {
  if (!headerValue) return false;
  const envKey = process.env.MODEL_SYNC_API_KEY ?? "";
...
export function checkRateLimit(
  key: string,
  max = Number(process.env.MODEL_SYNC_RATE_LIMIT_MAX ?? 30),
  windowMs = Number(process.env.MODEL_SYNC_RATE_LIMIT_WINDOW_MS ?? 60_000),
